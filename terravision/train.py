...
CROP_REGISTRY = {
    "Wheat":   {"yield_range": (1.5,  7.0),  "temp_K": 291.5, "moisture": 0.025,
                "ndvi_mean": 0.40, "ndvi_std": 0.15},
    "Rice":    {"yield_range": (2.0,  8.5),  "temp_K": 298.2, "moisture": 0.085,
                "ndvi_mean": 0.50, "ndvi_std": 0.18},
    "Maize":   {"yield_range": (2.5, 12.5),  "temp_K": 295.0, "moisture": 0.045,
                "ndvi_mean": 0.55, "ndvi_std": 0.20},
    "Soybean": {"yield_range": (1.2,  5.5),  "temp_K": 296.5, "moisture": 0.055,
                "ndvi_mean": 0.45, "ndvi_std": 0.16},
}
...
def generate_v1_dataset(
    ...
    for cfg in CROP_REGISTRY.values():
        ...
        y_range  = y_max - y_min
        y = (y_min + y_range * np.clip(ndvi, 0, 1)
             * np.exp(-((temp_K - cfg["temp_K"]) ** 2) / (2 * 8.0 ** 2))
             * np.clip(moisture / cfg["moisture"], 0.6, 1.2)
             + rng.normal(0, 0.30, n_per)).clip(0, YIELD_MAX)
...
def generate_v2_dataset(
    ...
    for cfg in CROP_REGISTRY.items():
        ...
        y_crop = (
            y_min
            + y_range * np.clip(peak_ndvi, 0, 1)
            * np.exp(-((c["temp_K"] - c["temp_K"]) ** 2) / (2 * 8.0 ** 2))
            + rng.normal(0, 0.30, n)
        ).clip(0, YIELD_MAX).astype(np.float32)
...
def train_model(
    ...
    log.info("Training %s for up to %d epochs | train=%d val=%d test=%d",
    ...
    for ep in range(1, epochs + 1):
        ...
        if v_mse < best_val - 1e-5:
            ...
            pat_cnt += 1
            if pat_cnt >= patience:
                log.info("  Early stop at epoch %d (best val=%.4f at ep %d)",
                         ep, best_val, best_ep)
                break

    elapsed = time.time() - t0
    log.info("Training done in %.1f s — best val_MSE=%.4f at epoch %d", elapsed, best_val, best_ep)
    ...
    log.info("Test  → MSE=%.4f  RMSE=%.4f t/ha  MAE=%.4f t/ha  R²=%.4f",
    ...
    log.info("MC Dropout (30 passes) → σ=±%.3f t/ha  95%% CI ±%.3f t/ha",
             conf["std_yield"], (conf["ci_95_upper"] - conf["ci_95_lower"]) / 2)
...
def eval_only(model_version: str = "v2") -> None:
    ...
    log.info("Eval → MSE=%.4f RMSE=%.4f MAE=%.4f R²=%.4f",
             metrics["mse"], metrics["rmse"], metrics["mae"], metrics["r2"])
    conf = mc_dropout_confidence(m, X_norm[:64], n_passes=30)
    log.info("MC Dropout → σ=±%.3f t/ha  conf=%.1f %%",
...
def main() -> None:
    ...
    if args.eval_only:
        ...
        train(args.model, args.n, args.epochs, args.lr, args.batch, args.patience, args.seed)
